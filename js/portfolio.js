...
;(function ( $, window, undefined ) {
	...
	Portfolio.prototype.setupThumbnails = function () {
		...
		var self = this,
				...
				iconLoading = $('<i/>',{'class': 'icon-remove-circle icon-spin'}),
	...
	Portfolio.prototype.rearrangeItems = function (detailedThumb) {
		...
		if (detailedThumb){
			...
			arrow.css({
				left: (this.detailedThumbIndex % this.options.thumbsPerRow) * thumbWidth + thumbWidth * 0.5 - parseInt(arrow.css('border-left-width'), 10)
			}).end().css({
				'z-index': '',
				opacity: sameRow? 1:0,
